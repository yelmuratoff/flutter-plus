...
import { dataClass } from "./commands/data.command";
...
export async function activate(
  ...
  if (!dartExt) {
    vscode.window.showErrorMessage(
      "The Dart extension is not installed. Flutter extension cannot be activated."
    );
    return;
  }
  ...
  if (!dartExt.exports) {
    console.error(
      "The Dart extension did not provide an exported API. It may have failed to activate or is not the latest version."
    );
    return;
  }
  ...
  if (!flutterExt) {
    vscode.window.showErrorMessage(
      "The Flutter extension is not installed. Flutter Plus extension cannot be activated."
    );
    return;
  }
...
function registerCommands(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("flutter-plus.data", dataClass)
  );
}
